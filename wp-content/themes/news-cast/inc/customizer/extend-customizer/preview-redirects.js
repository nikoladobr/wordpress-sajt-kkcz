 ...
 */
 ...
 (function ( api ) {
    api.section( 'static_front_page', function( section ) {
        news_cast_redirect_preview_url( section, api.settings.url.home )
    });

    api.section( 'innerpages_error_page_section', function( section ) {
        news_cast_redirect_preview_url( section, api.settings.url.home + '404pagenotfound' )
    });

    api.section( 'innerpages_search_page_section', function( section ) {
        news_cast_redirect_preview_url( section, api.settings.url.home + '?s=a' )
    });

    function news_cast_redirect_preview_url( section, url ) {
