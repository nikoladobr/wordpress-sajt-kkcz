<?php
/**
 * Post Grid Alter Element
 * 
 * @package News Cast
 * @since 1.0.0
 * 
 */
class Style_Mag_Post_Grid_Alter_Element extends \Elementor\Widget_Base {

    /**
     * @return - name of the widget
     */
    public function get_name() {
        return 'post-grid-alter';
    }
    ...
    public function get_title() {
        return esc_html__( 'Post Grid Alter', 'news-cast' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default'   => esc_html__( 'News Cast Post Grid Alter Title', 'news-cast' ),
    ...
    protected function render() {
        ...
        echo '<div id="bmm-post-grid-alter-block-'.esc_attr( $element_id ).'" class="bmm-post-grid-alter-block block-'.esc_attr( $element_id ).' bmm-block bmm-block-post-grid-alter--layout-one bmm-block-image-hover--'.esc_html( $imageHoverType ).'">';
            if( !empty( $blockTitle ) ) {
                echo '<h2 class="bmm-block-title"><span>'.esc_html( $blockTitle ).'</span></h2>';
            }
            // include template file w.r.t  layout value
            include( NEWS_CAST_INCLUDES_PATH .'elementor-widgets/post-grid-alter/template.php' );
        echo '</div><!-- #bmm-post-grid-alter-block -->';
    }
}